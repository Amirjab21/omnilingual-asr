...
from omnilingual_asr.models.lora import LoraConfig, inject_lora, LoRALinear
...
class TrainingConfig:
    ...
    mapping_dialect_to_lang: Dict[str, str] = field(default_factory=lambda: {
        "UAE": "afb_Arab",
        "Egypt": "arz_Arab",
        "Algeria": "arq_Arab",
    })
    ...
    lora_targets: Tuple[str, ...] = ("llama_decoder","encoder_frontend","encoder","encoder_proj", "final_proj", "lang_embeddings", "text_frontend")
    ...
    def to_dict(self) -> Dict:
        return {k: str(v) if isinstance(v, torch.device) else v for k, v in dataclasses.asdict(self).items()}
    ...
    def __init__(self, config: TrainingConfig, split: str) -> None:
        ...
        for cfg_name in config.dataset_configs:
            try:
                ...
                lang_code = config.mapping_dialect_to_lang.get(cfg_name, config.default_lang_code)
                ds_cfg = ds_cfg.map(lambda x, c=cfg_name, l=lang_code: {
                    "config_name": c,
                    "lang_code": l
                })
                ds_list.append(ds_cfg)
            except Exception as e:
                print(f"Warning: Could not load config {cfg_name} for split {split}: {e}")
...
class OmniASRCollator:
    ...
    def __call__(self, batch: List[Dict[str, object]]) -> Seq2SeqBatch:
        ...
        target_seqs = torch.full((len(batch), max_tgt), self.pad_idx, dtype=tokens[0].dtype, device=self.device)
...
def setup_model(config: TrainingConfig):
    ...
    model, tokenizer = load_llm_asr_300m(lang_embeddings_p=0.5, device=config.device)
    ...
    print(f"Model loaded on {config.device}. Total params: {total_params:,}. Trainable (LoRA): {trainable_params:,}")
...
class OmniTrainer:
    def __init__(self, config: TrainingConfig, model: torch.nn.Module, tokenizer: object):
        self.config = config
        self.model = model
        self.tokenizer = tokenizer
    ...
    def validate(self, dataloader: DataLoader, log_examples: bool = False):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader, desc="Validating", leave=False)):
                ...
                loss, _, _, decoder_context_inputs, decoder_context_layout = self.model(batch, return_logits=True)
                total_loss += float(loss.item())
                num_batches += 1

                # Log examples
                if log_examples and batch_idx < self.config.eval_log_examples and self.beam_generator:
                    self._log_example(batch, decoder_context_inputs, decoder_context_layout, batch_idx)
    ...
    def _log_example(self, batch, decoder_context_inputs, decoder_context_layout, batch_idx):
...
def main():
    ...
    loader_kwargs = {
        ...
        "num_workers": config.num_workers_gpu if torch.cuda.is_available() else config.num_workers_cpu,
